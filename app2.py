        ...
        if result.get("recommendations"):
            ...
            for product in result["recommendations"]:
                ...
                with cols[0]:
                    ...
                    # Show placeholder image if URL is clearly broken or ends with a dead image file
                    if not img_url or not img_url.startswith("http") or img_url.endswith(".gif") or "I/" not in img_url:
                        st.image("https://via.placeholder.com/150?text=No+Image", width=140)
                    else:
                        try:
                            # Try displaying the image — fallback if it silently fails
